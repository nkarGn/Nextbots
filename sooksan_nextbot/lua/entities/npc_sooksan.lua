...
ENT.JumpSound = Sound("npc_sooksan/jump.mp3")
ENT.JumpHighSound = Sound("npc_sooksan/spring.mp3")
ENT.TauntSounds = {
	Sound("npc_sooksan/pieceofcake.mp3"),
	Sound("npc_sooksan/stepitup.mp3"),
	Sound("npc_sooksan/tooeasy.mp3"),
	Sound("npc_sooksan/tooslow.mp3"),
}
local chaseMusic = Sound("npc_sooksan/panic.mp3")
...
local npc_sooksan_acquire_distance =
	CreateConVar("npc_sooksan_acquire_distance", 2500, FCVAR_NONE,
	"The maximum distance at which sooksan will chase a target.")

local npc_sooksan_spawn_protect =
	CreateConVar("npc_sooksan_spawn_protect", 1, FCVAR_NONE,
	"If set to 1, sooksan will not target players or hide within 200 units of \z
	a spawn point.")

local npc_sooksan_attack_distance =
	CreateConVar("npc_sooksan_attack_distance", 80, FCVAR_NONE,
	"The reach of sooksan's attack.")

local npc_sooksan_attack_interval =
	CreateConVar("npc_sooksan_attack_interval", 0.2, FCVAR_NONE,
	"The delay between sooksan's attacks.")

local npc_sooksan_attack_force =
	CreateConVar("npc_sooksan_attack_force", 800, FCVAR_NONE,
	"The physical force of sooksan's attack. Higher values throw things \z
	farther.")

local npc_sooksan_smash_props =
	CreateConVar("npc_sooksan_smash_props", 1, FCVAR_NONE,
	"If set to 1, sooksan will punch through any props placed in their way.")

local npc_sooksan_allow_jump =
	CreateConVar("npc_sooksan_allow_jump", 1, FCVAR_NONE,
	"If set to 1, sooksan will be able to jump.")

local npc_sooksan_hiding_scan_interval =
	CreateConVar("npc_sooksan_hiding_scan_interval", 3, FCVAR_NONE,
	"sooksan will only seek out hiding places every X seconds. This can be an \z
	expensive operation, so it is not recommended to lower this too much. \z
	However, if distant sooksans are not hiding from you quickly enough, you \z
	may consider lowering this a small amount.")

local npc_sooksan_hiding_repath_interval =
	CreateConVar("npc_sooksan_hiding_repath_interval", 1, FCVAR_NONE,
	"The path to sooksan's hiding spot will be redetermined every X seconds.")

local npc_sooksan_chase_repath_interval =
	CreateConVar("npc_sooksan_chase_repath_interval", 0.1, FCVAR_NONE,
	"The path to and position of sooksan's target will be redetermined every \z
	X seconds.")

local npc_sooksan_expensive_scan_interval =
	CreateConVar("npc_sooksan_expensive_scan_interval", 1, FCVAR_NONE,
	"Slightly expensive operations (distance calculations and entity \z
	searching) will occur every X seconds.")

local npc_sooksan_force_download =
	CreateConVar("npc_sooksan_force_download", 1, FCVAR_ARCHIVE,
	"If set to 1, clients will be forced to download sooksan resources \z
...
if npc_sooksan_force_download:GetBool() then
	resource.AddWorkshop(workshopID)
end

util.AddNetworkString("sooksan_nag")
util.AddNetworkString("sooksan_navgen")
...
local VECTOR_sooksan_HEIGHT = Vector(0, 0, 96)
local function isPointSuitableForHiding(point)
	trace.start = point
	trace.endpos = point + VECTOR_sooksan_HEIGHT
...
local function buildHidingSpotCache()
	...
	print(string.format("npc_sooksan: found %d suitable (%d unsuitable) hiding \z
...
local function isValidTarget(ent)
	...
	-- Ignore dead NPCs, other sooksans, and dummy NPCs.
	local class = ent:GetClass()
	return (ent:IsNPC()
		and ent:Health() > 0
		and class ~= "npc_sooksan"
		and not class:find("bullseye"))
end

hook.Add("PlayerSpawnedNPC", "sooksanMissingNavmeshNag", function(ply, ent)
	if not IsValid(ent) then return end
	if ent:GetClass() ~= "npc_sooksan" then return end
	if navmesh.GetNavAreaCount() > 0 then return end

	-- Try to explain why sooksan isn't working.
	net.Start("sooksan_nag")
	net.Send(ply)
end)
...
local function navEndGenerate()
	...
	if not navmesh.IsGenerating() then
		print("npc_sooksan: Navmesh generation completed in " .. timeElapsedStr)
	else
		print("npc_sooksan: Navmesh generation aborted after " .. timeElapsedStr)
	end
...
local function navGenerate()
	...
	if next(seeds, nil) == nil then
		print("npc_sooksan: Couldn't find any places to seed nav_generate")
		return false
	end
	...
	for seed in pairs(seeds) do
		...
		if not tr.StartSolid and tr.Hit then
			print(string.format("npc_sooksan: Adding seed %s at %s", seed, pos))
			navmesh.AddWalkableSeed(tr.HitPos, tr.HitNormal)
		else
			print(string.format("npc_sooksan: Couldn't add seed %s at %s", seed,
				pos))
		end
	end

	-- The least we can do is ensure they don't have to listen to this noise.
	for _, sooksan in pairs(ents.FindByClass("npc_sooksan")) do
		sooksan:Remove()
	end
	...
	if navmesh.IsGenerating() then
		...
		hook.Add("ShutDown", "sooksanNavGen", navEndGenerate)
	else
		print("npc_sooksan: nav_generate failed to initialize")
...
concommand.Add("npc_sooksan_learn", function(ply, cmd, args)
	if navmesh.IsGenerating() then
		return
	end
	...
	if game.SinglePlayer() then
		print("npc_sooksan: Beginning nav_generate requested by " .. ply:Name())

		-- Disable expensive computations in single-player. sooksan doesn't use
	...
	elseif isConsole then
		print("npc_sooksan: Beginning nav_generate requested by server console")
	else
		return
	end
...
function ENT:GetNearestTarget()
	...
	local maxAcquireDist = npc_sooksan_acquire_distance:GetInt()
	...
	for _, ent in pairs(acquirableEntities) do
		...
		-- if `npc_sooksan_spawn_protect' = 1.
		--TODO: Only for the first few seconds?
		if npc_sooksan_spawn_protect:GetBool() and ent:IsPlayer()
...
function ENT:AttackNearbyTargets(radius)
	local attackForce = npc_sooksan_attack_force:GetInt()
	...
	for _, ent in pairs(nearEntities) do
		if isValidTarget(ent) then
			...
			-- Give the player a good whack. sooksan means business.
		...
		elseif ent:GetMoveType() == MOVETYPE_VPHYSICS then
			if not npc_sooksan_smash_props:GetBool() then continue end
...
function ENT:AttemptJumpAtTarget()
	...
	local maxAttackDistance = npc_sooksan_attack_distance:GetInt()
...
function ENT:BehaveUpdate() --TODO: Split this up more. Eww.
	...
	local scanInterval = npc_sooksan_expensive_scan_interval:GetFloat()
	...
	if IsValid(self.CurrentTarget) then
		...
		local attackInterval = npc_sooksan_attack_interval:GetFloat()
		if currentTime - self.LastAttack > attackInterval then
			local attackDistance = npc_sooksan_attack_distance:GetInt()
		...
		local repathInterval = npc_sooksan_chase_repath_interval:GetFloat()
		...
		if self:IsOnGround() and npc_sooksan_allow_jump:GetBool()
		...
		local hidingScanInterval = npc_sooksan_hiding_scan_interval:GetFloat()
		...
		if self.HidingSpot ~= nil then
			local hidingInterval = npc_sooksan_hiding_repath_interval:GetFloat()
...
local MAT_sooksan = Material("npc_sooksan/sooksan")
killicon.Add("npc_sooksan", "npc_sooksan/killicon", color_white)
language.Add("npc_sooksan", "sooksan ")
...
local function DevPrint(devLevel, msg)
	if developer:GetInt() >= devLevel then
		print("npc_sooksan: " .. msg)
	end
end

local panicMusic = nil
local lastPanic = 0 -- The last time we were in music range of a sooksan.

--TODO: Why don't these flags show up? Bug? Documentation would be lovely.
local npc_sooksan_music_volume =
	CreateConVar("npc_sooksan_music_volume", 1,
	bit.bor(FCVAR_DEMO, FCVAR_ARCHIVE),
	"Maximum music volume when being chased by sooksan. (0-1, where 0 is muted)")

-- If another sooksan comes in range before this delay is up,
-- the music will continue where it left off.
local MUSIC_RESTART_DELAY = 2

-- Beyond this distance, sooksans do not count to music volume.
local MUSIC_CUTOFF_DISTANCE = 1000

-- Max volume is achieved when MUSIC_sooksan_PANIC_COUNT sooksans are this close,
-- or an equivalent score.
local MUSIC_PANIC_DISTANCE = 200

 -- That's a lot of sooksan.
local MUSIC_sooksan_PANIC_COUNT = 8

local MUSIC_sooksan_MAX_DISTANCE_SCORE =
	(MUSIC_CUTOFF_DISTANCE - MUSIC_PANIC_DISTANCE) * MUSIC_sooksan_PANIC_COUNT

local function updatePanicMusic()
	if #ents.FindByClass("npc_sooksan") == 0 then
		-- Whoops. No need to run for now.
		DevPrint(4, "Halting music timer.")
		timer.Remove("sooksanPanicMusicUpdate")
	...
	local userVolume = math.Clamp(npc_sooksan_music_volume:GetFloat(), 0, 1)
	...
	for _, ent in pairs(nearEntities) do
		if IsValid(ent) and ent:GetClass() == "npc_sooksan" then
	...
	local musicVolume = math.min(1,
		totalDistanceScore / MUSIC_sooksan_MAX_DISTANCE_SCORE)
	...
	if musicVolume > 0 then
		...
		if not LocalPlayer():Alive() then
			-- Quiet down so we can hear sooksan taunt us.
...
local function startTimer()
	if not timer.Exists("sooksanPanicMusicUpdate") then
		timer.Create("sooksanPanicMusicUpdate", 0.05, REPEAT_FOREVER,
			updatePanicMusic)
...
function ENT:DrawTranslucent()
	render.SetMaterial(MAT_sooksan)

	-- Get the normal vector from sooksan to the player's eyes, and then compute
	...
	-- sooksan should only look 1/3 of the way up to the player so that they
...
surface.CreateFont("sooksanHUD", {
	font = "Arial",
	size = 56
})

surface.CreateFont("sooksanHUDSmall", {
	font = "Arial",
	size = 24
})
...
local function navGenerateHUDOverlay()
	draw.SimpleTextOutlined("sooksan is studying this map.", "sooksanHUD",
		ScrW() / 2, ScrH() / 2, color_white,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 2, color_black)
	draw.SimpleTextOutlined("Please wait...", "sooksanHUD",
		ScrW() / 2, ScrH() / 2, color_white,
	...
	draw.SimpleTextOutlined("Time Elapsed:", "sooksanHUDSmall",
		ScrW() / 2, ScrH() * 3/4, color_white,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_black)
	draw.SimpleTextOutlined(elapsedStr, "sooksanHUDSmall",
		ScrW() / 2, ScrH() * 3/4, color_white,
	...
	draw.SimpleTextOutlined(flavourText, "sooksanHUDSmall",
		ScrW() / 2, ScrH() * 4/5, color_yellow,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)
end

net.Receive("sooksan_navgen", function()
	local startSuccess = net.ReadBool()
	if startSuccess then
		generateStart = SysTime()
		lastBracket = 0
		hook.Add("HUDPaint", "sooksanNavGenOverlay", navGenerateHUDOverlay)
	else
		Derma_Message("Oh no. sooksan doesn't even know where to start with \z
		this map.\n\z
...
local function requestNavGenerate()
	RunConsoleCommand("npc_sooksan_learn")
end
...
local function navWarning()
	Derma_Query("It will take a while (possibly hours) for sooksan to figure \z
		this map out.\n\z
...
net.Receive("sooksan_nag", function()
	if not nagMe then return end

	if game.SinglePlayer() then
		Derma_Query("Uh oh! sooksan doesn't know this map.\n\z
			Would you like him to learn it?",
			"This map is not yet sooksan-compatible!",
			"Yes", navWarning,
			"No", nil,
			"No. Don't ask again.", stopNagging)
	else
		Derma_Query("Uh oh! sooksan doesn't know this map. \z
			He won't be able to move!\n\z
			Because you're not in a single-player game, he isn't able to \z
			learn it.\n\z
			\n\z
			Ask the server host about teaching this map to sooksan.\n\z
			\n\z
			If you ARE the server host, you can run npc_sooksan_learn over \z
			rcon.\n\z
			Keep in mind that it may take hours during which you will be \z
			unable\n\z
			to play, and THE MAP WILL BE RESTARTED.",
			"This map is currently not sooksan-compatible!",
			"Ok", nil,
